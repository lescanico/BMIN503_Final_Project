...
This project investigates psychiatry resource utilization at the Penn Behavioral Health (PBH) Outpatient Psychiatry Clinic (OPC). The primary goal is to develop a predictive model for resource utilization.

The analysis integrates two datasets from the Epic Analytics database spanning a 10-year period: one focused on patient based data, and the other on individual visits. The project aims to uncover patterns in appointment utilization and resource demands.
...
The project’s GitHub repository can be found here: <https://github.com/lescanico/BMIN503_Final_Project>.
...
This project seeks to address these challenges by analyzing patterns in psychiatry resource utilization. Insights from the analysis will inform a predictive model designed to:
...
-   **Data Science & Predictive Modeling:** Supplies tools for analyzing complex data and developing predictive models to uncover patterns in resource utilization.

-   **Operations Management:** Offers insights into clinic workflows, resource allocation, and real-world feasibility for balancing patient demand with resource availability.
...
Patient and visit data from the PBH OPC spanning 10/1/2014 to 9/30/2024 was extracted from Epic Analytics. The data was securely downloaded into a private, access-controlled location outside the project repository. The extracted files, originally in `.xlsx` format, were converted to `.csv` format and stored securely as `patient_data.csv` and `visit_data.csv` within the same environment.
...
plot_missing_values_by_type(data_preprocessed, "Preprocessed Data", min_missing = 0, max_missing = 1)
plot_missing_values_by_type(data_preprocessed, "Preprocessed Data", min_missing = 1, max_missing = 5)
plot_missing_values_by_type(data_preprocessed, "Preprocessed Data", min_missing = 5, max_missing = 30)
plot_missing_values_by_type(data_preprocessed, "Preprocessed Data", min_missing = 30, max_missing = 50)
plot_missing_values_by_type(data_preprocessed, "Preprocessed Data", min_missing = 50, max_missing = 100)
```
...
A structured and comprehensive set of options for handling missing data across various variable types and levels of missingness was created. These strategies are tailored to address different percentages of missing data, ranging from minimal (0-5%) to very high (\>50%), and are organized in descending order of preference from the most ideal to the least ideal approach. Each strategy is listed individually for clarity and ease of implementation.
...
A comprehensive workflow was developed for generating a summary table that highlights missing data statistics and suggests handling strategies based on variable types and missingness levels. The workflow includes:

-   **Summary Statistics Calculation**: Computes descriptive statistics for variables based on their types, such as mean, median, skewness, and kurtosis for numeric data or most common levels for categorical data.

-   **Missingness Analysis**: Identifies the percentage of missing data for each variable and categorizes them into levels (e.g., Low, Moderate, High, Very High).

-   **Customizable Handling Strategies**: Allows the user to manually select, list, or randomly assign handling strategies for each variable depending on its type and missingness level.

The output is presented as a styled HTML table using `kableExtra` for clear visualization.
...
generate_data_handling_table <- function(dataset) {
  ...
  table <- missing_data_summary %>%
    ...
    kbl(caption = "Missing Data Handling Table", align = c("r", "c", "l", "l"), escape = FALSE) %>%
...
<iframe src="figures/missing_data_handling_table.html" width="100%" height="600">

</iframe>

### Data Reduction

A simplified `reduced_data_preprocessed` dataset of 20 high-yield variables will be created for an initial exploratory iteration.

### Missing Data Handling

Will implement chosen strategies.

### Exploratory Data Analysis (EDA)

Will analyze and summarize datasets to uncover patterns, detect anomalies, test hypotheses, and validate assumptions before building predictive models.

### Data Transformation

#### Normalization & Scaling

Data will be normalized to ensure an even contribution to the model by each feature, scaling them for magnitude-sensitive algorithms.

#### Encoding

Categorical variables will be converted into numerical format for downstream machine learning processing.

### Feature Engineering

A composite Resource Utilization Score (RUS) will be developed from the following adjusted, normalized and weighted features per `patient_id`:
...
-   Low Utilization of Resources (LUR)

-   Medium Utilization of Resources (MUR)

-   High Utilization of Resources (HUR)
...
One or more advanced models will be selected.
...
-   Will summarize key statistics of the datasets used for modeling.

-   Will provide insights into the target variable (RUS) distribution.
...
Will determines which features contribute most to the predictive power of the model.

### Validation and Testing

Will evaluate the final model using unseen data to estimate its generalizability.

## Conclusion {#sec-conclusion}

Will discuss key findings, limitations, generalizability and scalability.
