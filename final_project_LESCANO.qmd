...
# Read the HTML file and preserve line breaks
html_lines <- readLines("outputs/data_renaming.html", warn = FALSE)

# Combine the lines into a single HTML string
html_content <- HTML(paste(html_lines, collapse = "\n"))
...
# Output the wrapped HTML content as raw HTML
...
capture_output_to_html <- function(file, title, ...) {
    ...
    for (section_name in names(args)) {
        ...
        if (is.data.frame(args[[section_name]])) {
            # Handle data frames within scrollable div
            ...
            # Handle image paths by embedding Base64 images
            for (img_path in args[[section_name]]) {
                # Check if the image file exists
                if (file.exists(img_path)) {
                    # Encode the image as Base64
                    encoded_image <- base64encode(file(img_path, "rb"))
                    # Determine the MIME type of the image
                    mime_type <- ifelse(grepl("\\.png$", img_path, ignore.case = TRUE), "image/png",
                                 ifelse(grepl("\\.jpg$|\\.jpeg$", img_path, ignore.case = TRUE), "image/jpeg",
                                 ifelse(grepl("\\.svg$", img_path, ignore.case = TRUE), "image/svg+xml", "application/octet-stream")))
                    # Embed the image in the HTML
                    cat(sprintf('<img src="data:%s;base64,%s" style="max-width:100%%; height:auto;">\n', mime_type, encoded_image), file = con)
                } else {
                    cat(sprintf("<p>Image not found: %s</p>\n", img_path), file = con)
                }
            }
        } else {
...
generate_data_handling_table <- function(dataset, dataset_name = deparse(substitute(dataset))) {
  ...
  data_handling_table <- missing_data_summary %>%
    mutate(
      ...
      missing_percentage = ifelse(
        missing_percentage < 0.1, "<0.1%", sprintf("%.1f%%", missing_percentage)
      )
    ) %>%
    arrange(missing_percentage_numeric) %>%
    select(variable, missing_percentage, summary_stats, handling_strategy) %>%
    rename(
      Variable = variable,
