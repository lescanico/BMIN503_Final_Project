...
# Capture as HTML table [See Appendix - Helper Functions](#capture-output-as-html)
...
# Capture Summary of Preprocessed Data [See Appendix - Helper Functions](#capture-output-as-html)
source("scripts/helper-functions/capture-to-html.R")
capture_output_to_html(
...
```{r, results='asis', echo=FALSE}
cat(readLines("outputs/data_preprocessed_head.html"), sep = "\n")
```
...
# Visualize Missing Data [See Appendix - Helper Functions](#capture-output-as-html)
...
# Generate the data handling table [See Appendix - Helper Functions](#generate-data-handling-table)
...
# Capture as tables in HTML [See Appendix - Helper Functions](#capture-output-as-html)
source("scripts/helper-functions/capture-to-html.R")
...
```{r, results='asis', echo=FALSE}
cat(readLines("outputs/data_preprocessed_handling_table_all_options.html"), sep = "\n")
```
...
```{r, eval=FALSE}
# Facilitate variable choosing

# Step 1: Create a data frame from the colnames(data_preprocessed)
colnames_data <- colnames(data_preprocessed)

# Step 2: Filter variable_type_lookup to include only variables present in colnames(data_preprocessed)
filtered_lookup <- variable_type_lookup[variable_type_lookup$Variable %in% colnames_data, ]

# Step 3: Calculate missing percentages for each variable in data_preprocessed
missingness <- colSums(is.na(data_preprocessed)) / nrow(data_preprocessed) * 100
missingness_df <- data.frame(Variable = colnames(data_preprocessed), Missingness = round(missingness, 2))

# Step 4: Merge the filtered lookup table with the missingness data
result_df <- merge(filtered_lookup, missingness_df, by = "Variable", all.x = TRUE)

# Step 5: Sort the resulting data frame by Missingness ascendingly
result_df <- result_df[order(result_df$Missingness), ]

# Step 6: Define detailed logical groups
logical_groups <- list(
  ...
  Geographic_Info = c("country", "state", "patient_county", "visit_county", "patient_postal_code", "visit_postal_code", "rural_urban_commuting_area_primary", "rural_urban_commuting_area_secondary", "svi_2020_socioeconomic_percentile_census_tract"),
  ...
  Financial = c("primary_benefit_plan", "primary_payer", "primary_payer_financial_class", "self_pay", "copay_collected", "copay_due", "prepayment_due", "prepayment_collected", "primary_subscriber_group_number", "patient_service_level", "visit_service_level"),
  Health_Metrics = c("patient_bmi", "visit_bmi", "patient_bp_diastolic", "visit_bp_diastolic", "patient_bp_systolic", "visit_bp_systolic", "phq_2_total_score", "phq_9", "general_risk_score", "sdoh_risk_level", "sdoh_domains", "adi_national_percentile", "adi_state_decile"),
  Clinical_History = c("patient_diagnosis", "visit_diagnosis", "primary_diagnosis", "medical_history", "allergies_and_contraindications"),
  Treatments = c("medications", "outpatient_medications", "patient_medications_ordered", "visit_medications_ordered", "hospital_or_clinic_administered_medications", "procedures", "visit_procedures_ordered", "patient_procedures_ordered"),
  Patient_Portal = c("portal_active_at_scheduling", "mychart_status"),
  Patient_Status = c("new_to_department_specialty", "new_to_facility", "new_to_provider", "university_of_pennsylvania_student")
)

# Step 7: Map each variable to its logical group
group_mapping <- stack(logical_groups)
colnames(group_mapping) <- c("Variable", "Group")

# Step 8: Merge the `result_df` with the `group_mapping`
result_df <- merge(result_df, group_mapping, by = "Variable", all.x = TRUE)

# Step 9: Ensure the original order is preserved
result_df <- result_df[order(match(result_df$Variable, colnames(data_preprocessed))), ]

# Step 10: Reorder columns to make 'Group' the first column
result_df <- result_df[, c("Group", setdiff(colnames(result_df), "Group"))]
```
...
reduced_data_preprocessed <- data_preprocessed %>%
  ...
  select(
    ...
    # Demographics
    year_of_birth, legal_sex, marital_status, patient_ethnic_group, patient_race, patient_religion, adi_national_percentile,
    
    # Clinical Information
    allergies_and_contraindications, medical_history, 
    
    # Visit Information
    visit_date, appointment_length_minutes, appointment_status, primary_provider_type, priimary_provider_title, visit_service_level, primary_diagnosis, visit_diagnosis,
...
saveRDS(reduced_data_preprocessed, "datasets/reduced_data.rds")
```

## Reduced Iteration

```{r reduced-data-load, eval=FALSE}
# Load required Libraries
library(readr)

# Load reduced dataset
reduced_data <- readRDS("datasets/reduced_data.rds")
variable_type_lookup <- readRDS("datasets/variable_type_lookup.rds")
```

### Missing Data Handling

```{r reduced-missingness, eval=FALSE}
# Load required libraries
library(ggplot2)
library(reshape2)
library(dplyr)

# Function to create a heatmap for missing values
plot_missing_heatmap <- function(data, file_name) {
  # Convert data to a logical matrix of NAs
  na_matrix <- is.na(data)
  
  # Convert the matrix to a long-format data frame for ggplot
  na_long <- melt(na_matrix, varnames = c("Row", "Column"), value.name = "IsNA")
  
  # Create the heatmap
  p <- ggplot(na_long, aes(x = Column, y = Row, fill = IsNA)) +
    geom_tile() +
    scale_fill_manual(values = c("blue", "red"), labels = c("Present", "Missing"), name = "Missing") +
    theme_minimal() +
    theme(
...
plot_missing_heatmap(reduced_data, "figures/plots/missing_heatmap_reduced_data.png")
plot_missing_values_by_type(reduced_data, 0, 1)
plot_missing_values_by_type(reduced_data, 1, 5)
plot_missing_values_by_type(reduced_data, 5, 30)
plot_missing_values_by_type(reduced_data, 30, 50)
plot_missing_values_by_type(reduced_data, 50, 100)
```
...
```{r reduced-missing-data-handling-table, eval=FALSE}
# Generate the data handling table [See Appendix - Helper Functions](#generate-data-handling-table)
source("scripts/helper-functions/generate-missing-handling-table.R")
...
capture_output_to_html(
  ...
  # Create the heatmap
  ggplot(na_long, aes(x = Column, y = Row, fill = IsNA)) +
    geom_tile() +
    scale_fill_manual(values = c("white", "red"), labels = c("Present", "Missing"), name = "Missing") +
    theme_minimal() +
    theme(
      axis.text.x = element_text(angle = 90, hjust = 1),
      axis.text.y = element_blank(),
...
heatmap_plot <- plot_missing_heatmap(data_preprocessed)
```
