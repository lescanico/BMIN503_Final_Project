...
saveRDS(mrn_mapping, file = "H:/secure/mrn_mapping.rds")
...
# Helper function to summarize differences between two columns
summarize_column_differences <- function(col1, col2) {
  ...
  if (is.numeric(col1) && is.numeric(col2)) {
    abs_diff <- abs(col1 - col2)
    return(tibble(
      `Total Rows` = length(col1),
      `Different Rows` = sum(col1 != col2, na.rm = TRUE),
      `Proportion Different` = mean(col1 != col2, na.rm = TRUE),
      `Mean Difference` = mean(abs_diff, na.rm = TRUE),
      `SD Difference` = sd(abs_diff, na.rm = TRUE),
      `Max Difference` = max(abs_diff, na.rm = TRUE)
    ))
  } else if (is.character(col1) || is.logical(col1)) {
    return(tibble(
      `Total Rows` = length(col1),
      `Different Rows` = sum(col1 != col2, na.rm = TRUE),
      `Proportion Different` = mean(col1 != col2, na.rm = TRUE),
      `Unique Values Col1` = length(unique(na.omit(col1))),
      `Unique Values Col2` = length(unique(na.omit(col2))),
      `Overlap in Values` = length(intersect(unique(na.omit(col1)), unique(na.omit(col2))))
    ))
  } else {
    return(tibble(
      `Total Rows` = length(col1),
...
# Function to process and rename duplicate columns
process_and_rename_duplicates <- function(df, suffix_1 = "_from_patient_dataset", suffix_2 = "_from_visit_dataset") {
  ...
  for (col in cols_suffix_1) {
    ...
    if (corresponding_col %in% cols_suffix_2) {
      if (identical(df[[col]], df[[corresponding_col]])) {
        ...
        # Summarize differences for non-identical columns
        diff_summary <- tryCatch(
          summarize_column_differences(df[[col]], df[[corresponding_col]]),
          error = function(e) tibble(
            `Error` = as.character(e$message)
          )
        )
        integration_log[[paste0(col, " vs ", corresponding_col)]] <- diff_summary
        
        # Rename with patient_ and visit_ prefixes
        patient_col <- paste0("patient_", gsub(suffix_1, "", col))
        visit_col <- paste0("visit_", gsub(suffix_1, "", col))
        names(df)[names(df) == col] <- patient_col
        names(df)[names(df) == corresponding_col] <- visit_col
      }
    }
  }
  
  list(data = df, integration_log = integration_log)
}

# Apply the function to the merged dataset
processed_results <- process_and_rename_duplicates(merged_data)

# Extract processed data and log
deduplicated_data <- processed_results$data
difference_log <- processed_results$integration_log

# Print results
print("Processing complete.")
print("Differences between non-identical duplicates:")
print(difference_log)

# Save log
difference_log |> saveRDS("datasets/logs/deduplication.rds")
```

##### Organization

The merged and deduplicated dataset was reordered and sorted to prioritize relevant information. Columns were organized by their importance and relevance to the analysis objectives, and data was sorted primarily by patient ID and secondary by visit date. This organization facilitated easier access and manipulation of data for subsequent analyses.
...
logical_groups <- list(
  ...
  Demographic_Info = c("year_of_birth", "age_at_visit_years", "legal_sex", "patient_gender_identity", "visit_gender_identity", "patient_sex_assigned_at_birth", "visit_sex_assigned_at_birth", "patient_sexual_orientation", "visit_sexual_orientation", "patient_patient_race", "visit_patient_race", "patient_ethnic_group", "marital_status", "language", "patient_religion", "visit_religion"),
  
  Sociogeographic_Info = c("country", "state", "patient_country_county", "visit_country_county", "patient_postal_code", "visit_postal_code", "rural_urban_commuting_area_primary", "rural_urban_commuting_area_secondary", "svi_2020_socioeconomic_percentile_census_tract", "adi_national_percentile", "adi_state_decile"),
  
  Clinical_Info = c("primary_diagnosis", "patient_diagnosis", "visit_diagnosis", "medical_history", "allergies_and_contraindications"),
  
  Treatment_Info = c("procedures", "patient_procedures_ordered", "visit_procedures_ordered", "medications", "hospital_or_clinic_administered_medications", "outpatient_medications", "patient_medications_ordered", "visit_medications_ordered"),
  
  Health_Metrics = c("patient_bmi", "visit_bmi", "patient_bp_systolic", "visit_bp_systolic", "patient_bp_diastolic", "visit_bp_diastolic", "phq_2_total_score", "phq_9", "general_risk_score", "sdoh_risk_level", "sdoh_domains"),
  
  Encounter_Info = c("visit_date", "visit_type", "continuity_of_care", "encounter_type", "chief_complaint", "interpreter_needed", "appointment_status", "appointment_creation_date", "appointment_time", "appointment_length_minutes", "lead_time_days", "encounter_to_close_day", "scheduling_source", "no_show_probability"),
  
  Provider_Info = c("primary_provider_type", "primary_provider_title", "time_physician_spent_pre_charting_minutes", "time_physician_spent_post_charting_minutes", "time_with_physician_minutes", "time_waiting_for_physician_minutes"),
  
  Financial_Info = c("primary_benefit_plan", "primary_payer", "primary_payer_financial_class", "self_pay", "copay_due", "copay_collected", "prepayment_due", "prepayment_collected", "primary_subscriber_group_number", "patient_level_of_service", "visit_level_of_service"),
...
# Reorganize data
data_organized <- deduplicated_data |>
...
updated_mapping <- variable_type_mapping |> update_type_mapping(data_organized)
...
plot_missing_values_by_type(data_organized, 0, 1)
plot_missing_values_by_type(data_organized, 1, 5)
plot_missing_values_by_type(data_organized, 5, 30)
plot_missing_values_by_type(data_organized, 30, 50)
plot_missing_values_by_type(data_organized, 50, 100)
```

> See [Function to Plot Missing Values by Type](#plot-missing-values-by-type).

::: scrollable-inner-container
![](figures/plots/0-1_missing_data_organized_plot.png)

![](figures/plots/1-5_missing_data_organized_plot.png)\
![](figures/plots/30-50_missing_data_organized_plot.png)\
![](figures/plots/50-100_missing_data_organized_plot.png)\
:::
...
# Drop problematic 'sdoh_risk_level' variable
aggregated_data <- data_organized |> select(-sdoh_risk_level)

# Define list_vars: Identify list variables
list_vars <- names(aggregated_data)[sapply(aggregated_data, is.list)]

# Variables with specific aggregation rules
special_rules <- c("phq_2_total_score", "phq_9")

# Function to flatten nested lists into a single vector
...
aggregated_data[list_vars] <- lapply(list_vars, \(var) {
  if (var %in% special_rules) {
    # Apply specific aggregation rules
    sapply(aggregated_data[[var]], \(x) {
      x <- flatten_list(x)  # Flatten nested structures
      if (is.null(x) || all(is.na(x))) {
        NA
      } else {
        x_numeric <- suppressWarnings(as.numeric(x))
        if (all(is.na(x_numeric))) {
          NA  # If all values are non-numeric, return NA
        } else {
          switch(
            var,
            "phq_2_total_score" = mean(x_numeric, na.rm = TRUE),
            "phq_9" = mean(x_numeric, na.rm = TRUE)
          )
        }
      }
    })
    ...
    sapply(aggregated_data[[var]], \(x) {
      x <- flatten_list(x)  # Flatten nested structures
      if (is.null(x) || all(is.na(x))) {
        0  # NA or NULL means 0
      } else {
        length(unique(na.omit(x)))  # Count unique non-NA values
      }
    })
  }
})
...
aggregated_data_encoded |>
  saveRDS("datasets/data_encoded.rds")

# Save the encoding mappings
dir.create("datasets/mappings", recursive = TRUE, showWarnings = FALSE)
encoding_mappings |>
  saveRDS("datasets/mappings/encoding.rds")
...
impute_variable <- function(variable, var_type, var_name = NULL) {
  if (var_type == "numeric") {
    ...
    if (mean(is.na(variable)) < 0.1) {
      ...
      imputed_data <- mice(temp_data, m = 1)
      return(mice::complete(imputed_data)$var)
    }
...
imputed_data <- aggregated_data_encoded %>%
  mutate(across(
    everything(),
    ~ impute_variable(.x, 
...
saveRDS(imputed_data, "datasets/imputed/data_imputed.rds")

# Print confirmation
cat("Imputation complete. Imputed dataset saved to 'datasets/imputed/data_imputed.rds'.\n")

# Check for missing values across the dataset
missing_summary <- colSums(is.na(imputed_data))
...
final_data <- cbind(identifiers, modeling_data)

# Save the prepared dataset for modeling
saveRDS(final_data, "datasets/data_final_for_modeling_with_id.rds")
...
saveRDS(updated_mapping, "datasets/mappings/final_types.rds")
...
final_data <- readRDS("datasets/data_final_for_modeling_with_id.rds")
...
# Feature Engineering: Weekly Load (Minutes)
weekly_load_data <- final_data |>
  group_by(patient_id) |>
  summarise(
...
saveRDS(weekly_load_data, "datasets/engineered_features/weekly_load_data.rds")
