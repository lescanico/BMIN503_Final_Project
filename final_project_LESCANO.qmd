...
# Select columns without any missing values
non_na_cols <- colnames(data_featured)[colSums(is.na(data_featured)) == 0]
data_model <- data_featured |>
  select(all_of(non_na_cols))

# Ensure `visits_per_year` is in the dataset for modeling
if (!"visits_per_year" %in% colnames(data_model)) {
  stop("visits_per_year is missing from non-NA columns.")
}

# Filter for relevant data and ensure no missing values
