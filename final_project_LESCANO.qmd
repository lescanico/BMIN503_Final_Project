...
> See [Function to Capture Output as HTML].
...
> See [Function to Generate Missing Data Handling Strategies] and [Function to Calculate Summary Statistics].
...
# Create a data frame from the colnames(data_preprocessed)
colnames_data <- colnames(data_preprocessed)

# Filter variable_type_lookup to include only variables present in colnames(data_preprocessed)
filtered_lookup <- variable_type_lookup[variable_type_lookup$Variable %in% colnames_data, ]

# Calculate missing percentages for each variable in data_preprocessed
...
# Define detailed logical groups
logical_groups <- list(
...
# Generate one plot per missingness range
plot_list <- lapply(names(range_data), function(range) {
  ...
  create_plot(data, range, range_limits)
})
